...
export default function Slider() {
  ...
  const login =
    (useSelector((state: RootState) => state.user.user) as IUser) || null;
        ...
        {users &&
          user?.map((user, index) => (
            <div
              ...
              key={index}
              className="w-full flex items-center gap-5 px-5 h-[9vh] rounded-xl bg-zinc-100 hover:bg-zinc-200 cursor-pointer"
            >
              <div>
                <img
